...
#          attr(x, "expressions") <- expr
#        else       
#          attr(x, "expressions") <- c(attr(x, "expressions"), expr)
#        class(x) <- cl
#        return(x)
#    }
    if (!is.list(value)) {
...
#            v <- value[[jvseq[[jjj]]]]
            ...
            names(expr) <- jj
            if(is.null(attr(x, "expressions")))
              attr(x, "expressions") <- c(expr)
            else  
            {
              if(jj %in% names(attr(x, "expressions")))
                attr(x, "expressions")[which(jj == names(attr(x, "expressions")))] <- expr[[1]]
              else     
                attr(x, "expressions") <- c(expr, attr(x, "expressions"))
            }

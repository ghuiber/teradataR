td.kmeans <- function(x, centers, iter.max=10, nstart=1)
{
  ...
  for(i in 1:nstart)
  {
    ...
    {
      for(k in (j+1):centers)
        curD <- curD + sum(dist(testClusters[c(j,k),]))
    }
    if(curD > maxD)
    {
      maxD <- curD
  ...
  while(iter <= iter.max)
  {
    ...
    query <- gettextf("SELECT %s,%s FROM %s %s GROUP BY 1 ORDER BY 1", caseText,
                      sText, obj, wc)
    ...
    query <- gettextf("SELECT COUNT(*) FROM (SELECT %s, %s FROM %s WHERE clusterID <> newClusterID %s) A",
                      caseText, newCaseText, obj, ac)
  ...
  caseText <- .td.genclusterid(initClusters)
  query <- gettextf("SELECT %s, COUNT(*) FROM %s %s GROUP BY 1 ORDER BY 1", caseText,
                    obj, wc)
  sz <- try(tdQuery(query))
  csums <- paste(gettextf("SUM(C%d) AS C%d",1:centers,1:centers), collapse=",")
  subq <- gettextf("SELECT %s,%s FROM %s %s GROUP BY 1",
                    caseText,
                    paste(.td.genclusterss(initClusters),collapse=","), obj, wc)
...
.td.genclusterid <- function(centroids, AS="clusterID")
{
  ...
  caseText <- c("CASE")
  for(i in 1:ccnt)
    cText[i] <- paste("sqrt(",gettextf("(%f-\"%s\")**2", centroids[i,], nms), ")", sep="", collapse="+")
  ...
  if(ccnt > 1)
  {
    ...
    caseText <- c(caseText, gettextf(" WHEN (1=1) THEN 1 ", cText[1]))

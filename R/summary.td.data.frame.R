...
function(tdf, maxsum = 7,digits = max(3, getOption("digits") -
    3), ...)
{
  ...
  for(i in 1:length(names(tdf)))
  {
    ...
    {
#      query <- .td.genmedian(tdf,names(tdf)[i])
#      med <- tdQuery(query)
      query <- .td.genquartiles(tdf,names(tdf)[i], mmmn[[4]])
      quar <- tdQuery(query)
      vals <- c(mmmn[[1]],quar[[1]],quar[[3]],mmmn[[3]],quar[[2]],mmmn[[2]],
                if(mmmn[[5]] > 0) mmmn[[5]] else double(0))
      names(vals) <- c("Min.","1st Qu.","Median","Mean","3rd Qu.","Max",
                       if(mmmn[[5]] > 0) "NA's" else character(0))
    }
    else
    {
      vals <- c()
      names(vals) <- c()
      amt <- maxsum
      query <- gettextf("SELECT \"%s\", COUNT(*) as cnt FROM %s %s GROUP BY 1 ORDER BY 2 DESC",
                        names(tdf)[i], obj, wc)

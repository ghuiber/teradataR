td.tapply <- function (X, INDEX, FUN = NULL, asdf=FALSE, ...)
{
  ...
  if(!is.null(FUN))
  {
    ...
    if(length(fnc))
    {
      ...
      if(length(INDEX) > 2)
        srt <- paste("\"", names(INDEX)[2:length(INDEX)], "\"", sep="", collapse=",")
      else
        srt <- paste("\"", names(INDEX), "\"", sep="", collapse=",")
...
#      {
#        vals[[i]] <- as.vector(sort(unique(res[[i]])))
#        dms[i] <- length(vals[[i]])
#      }
#      names(vals) <- names(INDEX)
#      arr <- array(dim = dms, dimnames=vals)
#      for(i in 1:nrow(res))
#      {
#        mat <- matrix(as.character(res[i,1:length(dms)]), nrow=1)
#        arr[mat] <- res[i,length(res)]
#      }

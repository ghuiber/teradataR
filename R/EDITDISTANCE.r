EDITDISTANCE <- function(x, y)
{
  if(inherits(x, "td.data.frame"))
  {
    ...
    stop("Invalid data type for 'x' in EDITDISTANCE function")
  ...
  if(inherits(y, "td.data.frame"))
  {
    ...
    stop("Invalid data type for 'y' in EDITDISTANCE function")
...
#EDITDISTANCE <- function(x, second_string=" ")
#{
#  asTdExpr <- function(x) {class(x) <- "td.expression"; return(x)}
#
#  efmt <- "EDITDISTANCE(%s,%s)"
#  if(inherits(x, "td.data.frame"))
#  {
#    if(length(x) == 1)
#    {
...
#    }
#
#    return(asTdExpr(gettextf(efmt, val, second_string)))
#
#  }
#
